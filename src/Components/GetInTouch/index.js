...
const GetInTouch = (props) => {
  ...
  let saveToProfileData = (userInput) => {
    ...
    for (var property in textFieldObject) {
      error[property] = RegExp(regExObject[property], "g").test(
        userInput[property]
      )
        ? ""
        : errorFieldObject[property];
    ...
    axios
      ...
      .catch((err) => {
        console.log(" error from get in touch", err);
  ...
  let content = (
    <div className="col-md-12  padd0">
      {showModal ? (
        <AppModal
          isClosedFromAppModal={isClosedFromAppModal}
        ...
        {infoDataFromObserval.length !== 0 ? (
          infoDataFromObserval.map((ele, key) => {
